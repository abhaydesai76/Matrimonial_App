...
candidateRouter.put('/:id', async (request, response, next) => {
    try 
    {
        ...
        // If user has replaced profile photo, then only update it. If "photo" field includes a blob object, it implies
        // that user has explicitly updated profile photo from front end.
        
        if (body.photo.includes("blob") === true)
        {        
            candidatePhoto = request.protocol + '://' + request.get('host') + '/candidatePhotos/' + candidateID + '-' + body.profilePhoto            
        }
        else
        {         
            candidatePhoto = body.photo
        }        
